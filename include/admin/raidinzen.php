...
$design = new design ( 'Instanzen Erstellen/Bearbeiten', 'Erstellen/Bearbeiten', 2 );
$design->header();
...
$tpl = new raidTPL ( 'raid/raidinzen.htm',1 );
...
switch($menu->get(1)){ # `id`, `name`, `level`, `grpsize`, `img`, `info`, `maxbosse`
	...
	default:
		if( $menu->get(1) == '' ){
			...
			$tpl->db_array("info", "SELECT id, info FROM prefix_raid_info");
			$tpl->db_array("size", "SELECT id, grpsize FROM prefix_raid_grpsize");
		...
		if( $countFiles != 0 ){
			...
			while( $img = readdir( $open ) ){
				if( $img != "." and $img != ".." ){
					...
					$tpl->set_ar_out( array( "class" => "Cnorm", "img" => "<a class='group' href='".$imgPath.$img."'>".$img."</a>", "del" => $del),4);
				}
			}
			closedir();
		}else{
