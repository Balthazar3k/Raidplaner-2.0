...
switch($menu->get(1)){
		...
		{ 	case 1:
				...
				$res = db_query("SELECT 
								...
								FROM prefix_raid_dkp as a
									LEFT JOIN prefix_raid_charaktere AS b ON a.cid = b.id 
	...
	default:
		$db_table_breite = 50;
		echo "<br>";
		db_table( "SELECT a.klassen as Alle_Klassen, COUNT(b.klassen) as anzahl, a.id as del FROM prefix_raid_klassen AS a, prefix_raid_charaktere AS b 
		WHERE a.id=b.klassen GROUP BY a.klassen ORDER BY anzahl DESC", 0 );
		echo "<br>";
		db_table( "SELECT a.klassen as Alle_Main_Klassen, COUNT(b.klassen) as anzahl FROM prefix_raid_klassen AS a, prefix_raid_charaktere AS b 
		WHERE a.id=b.klassen AND b.rang>=4 GROUP BY a.klassen ORDER BY anzahl DESC", 0 );
		echo "<br>";
		db_table( "SELECT a.rassen, COUNT(b.rassen) as anzahl FROM prefix_raid_rassen AS a, prefix_raid_charaktere AS b 
		WHERE a.id=b.rassen GROUP BY a.rassen ORDER BY anzahl DESC", 0 );
		echo "<br>";
		db_table( "SELECT a.berufe as Main_Berufe, COUNT(b.mberuf) as anzahl FROM prefix_raid_berufe AS a, prefix_raid_charaktere AS b 
		WHERE a.id=b.mberuf GROUP BY a.berufe ORDER BY anzahl DESC", 0 );
		echo "<br>";
		db_table( "SELECT a.berufe as Second_Berufe, COUNT(b.sberuf) as anzahl FROM prefix_raid_berufe AS a, prefix_raid_charaktere AS b 
		WHERE a.id=b.sberuf GROUP BY a.berufe ORDER BY anzahl DESC", 0 );
		echo "<br>";
		db_table( "SELECT a.rang as Ränge, COUNT(b.rang) as anzahl FROM prefix_raid_rang AS a, prefix_raid_charaktere AS b 
